...
def construct_circuit(instrucs: list) -> None:
    series = pd.Series()
    ...
    while True:
        ...
        if len(leftover_instructions) == 0:
            ...
            instrucs = copy.deepcopy(leftover_instructions)
            leftover_instructions = []
...
# Part 1.
f = open(os.path.join(curr_dir, "input_day_07_part_1.txt"), "r")
instructions = f.read().split("\n")
f.close()
construct_circuit(instructions)


# Part 2.
f = open(os.path.join(curr_dir, "input_day_07_part_2.txt"), "r")
instructions = f.read().split("\n")
f.close()
construct_circuit(instructions)
