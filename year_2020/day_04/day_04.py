    ...
    if len(item) > 0:
        for field in item.split():
            ...
            if key == "byr":
                ...
                if unit == "cm":
                    if number.isdigit() and 150 <= int(number) <= 193:
                        fields_valid += 1

                elif unit == "in":
                    if number.isdigit() and 59 <= int(number) <= 76:
                        fields_valid += 1
                ...
                if regexp.search(value) and len(value) == 9:
                    fields_valid += 1

    else:
        if fields_missing == 0 and fields_valid == len(req_list):
            valid_count += 1
...
for item in txt_cleaned:
    fields_missing, fields_valid, valid_count = check_line(
        item, fields_missing, fields_valid, valid_count
    )
